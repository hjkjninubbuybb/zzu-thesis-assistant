...
def test_list_faqs_student_forces_approved(svc, mock_kb_store, mock_faq_store):
    ...
    mock_faq_store.list_faqs.return_value = ([], 0)

    svc.list_faqs("kb1", status="pending", role="student")

    mock_faq_store.list_faqs.assert_called_once_with("kb1", status="approved", page=1, page_size=20)
...
def test_list_faqs_admin_uses_original_status(svc, mock_kb_store, mock_faq_store):
    ...
    mock_faq_store.list_faqs.assert_called_once_with("kb1", status="pending", page=1, page_size=20)


def test_list_faqs_none_status_passes_through(svc, mock_kb_store, mock_faq_store):
    mock_kb_store.get_kb.return_value = {"name": "kb1"}
    mock_faq_store.list_faqs.return_value = ([{"id": 1}], 1)

    items, total = svc.list_faqs("kb1", status=None, role="teacher")

    mock_faq_store.list_faqs.assert_called_once_with("kb1", status=None, page=1, page_size=20)
    assert len(items) == 1
    assert total == 1
...
def test_create_admin_status_approved_and_vectors(svc, mock_kb_store, mock_faq_store):
    ...
    with patch("src.services.faq_service.service.upsert_faq_vector") as mock_upsert:
...
def test_create_vector_error_does_not_raise(svc, mock_kb_store, mock_faq_store):
    ...
    with patch("src.services.faq_service.service.upsert_faq_vector", side_effect=VectorStoreError("qdrant down")):
        # Should NOT raise
        result = svc.create("kb1", "q", "a", "cat", 0, 1, "admin")
...
def test_export_to_xlsx_returns_bytes_and_filename(svc, mock_kb_store, mock_faq_store):
    ...
    mock_kb_store.get_kb.return_value = {"name": "kb1"}
    mock_faq_store.list_faqs.return_value = ([{"id": 1, "question": "q", "answer": "a"}], 1)

    fake_wb = Workbook()
    with patch("src.services.faq_service._excel.build_faq_workbook", return_value=fake_wb):
