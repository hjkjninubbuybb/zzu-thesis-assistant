...
from src.core.shared.eval_base import EvaluatorOutput
...
def extractor_node(state: FormExtractionState) -> dict:
    ...
    if not state["attempts"]:
        ...
        # 重试：带上历史和 feedback
        history = ""
        for i, (attempt, feedback) in enumerate(zip(state["attempts"], state["feedback_history"])):
            if attempt:
                sections_text = "\n".join(f"- [{s['topic']}] {s['content']}" for s in attempt)
...
def evaluator_node(state: FormExtractionState) -> dict:
    ...
    if state["feedback_history"]:
        history = "\n\n".join(f"### 第 {i + 1} 轮反馈\n{fb}" for i, fb in enumerate(state["feedback_history"]))
        user_content += f"\n\n## 历史评估记录\n\n{history}"
