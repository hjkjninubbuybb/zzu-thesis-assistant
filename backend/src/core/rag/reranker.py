...
class Reranker(BaseReranker):
    ...
    def _rerank_batch(self, query: str, batch: list[dict]) -> list[dict]:
        ...
        reranked = reranker.postprocess_nodes(nws_list, query_bundle=QueryBundle(query_str=query))
        return [
            {
    ...
    def rerank(self, query: str, nodes: list[dict]) -> list[dict]:
        ...
        batches = [nodes[i : i + RERANK_BATCH_SIZE] for i in range(0, len(nodes), RERANK_BATCH_SIZE)]
        ...
        try:
            ...
            if len(batches) == 1:
                ...
                with ThreadPoolExecutor(max_workers=len(batches)) as executor:
                    futures = {executor.submit(self._rerank_batch, query, batch): batch for batch in batches}
