...
from src.core.interfaces.storage import BaseVectorStore
...
def fetch_corpus(kb_name: str, vector_store: BaseVectorStore) -> list[dict]:
...
class VectorRetriever(BaseRetriever):
    ...
    def __init__(self, kb_name: str, vector_store: BaseVectorStore, top_k: int = 10):
...
class HybridRetriever(BaseRetriever):
    ...
    def __init__(
        ...
        *,
        ...
        self._vector = VectorRetriever(kb_name, vector_store=vector_store, top_k=k_vector)
    ...
    def _rrf_fusion(self, vec_results: list[dict], bm25_results: list[dict]) -> list[dict]:
        ...
        return [{"score": s, **{k: v for k, v in r.items() if k != "score"}} for s, r in sorted_items]
