...
class FAQStore:
    ...
    def list_faqs(
        ...
        Returns:
            (FAQ 列表, 总条数) 元组，列表按 sort_order ASC, id ASC 排序。
        """
        where = "WHERE kb_name = %s"
        params: list = [kb_name]
        if enabled_only:
            where += " AND enabled = 1"
        if status:
        ...
        with get_conn() as conn, conn.cursor() as cur:
            cur.execute(f"SELECT COUNT(*) AS cnt FROM faqs {where}", params)
            total: int = cur.fetchone()["cnt"]
            offset = (page - 1) * page_size
            cur.execute(
                f"SELECT * FROM faqs {where} ORDER BY sort_order ASC, id ASC LIMIT %s OFFSET %s",
