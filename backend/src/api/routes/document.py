...
@router.get("/{kb_name}", response_model=PaginatedDocs)
def list_documents(
    kb_name: str,
    ...
    """列出知识库下的文档（可按 doc_type 过滤）。"""
    try:
        docs, total = svc.list_documents(kb_name, page=page, page_size=page_size, doc_type=doc_type)
    except KnowledgeBaseNotFoundError as e:
        raise HTTPException(status_code=404, detail=str(e)) from e
    return {"items": [DocInfo(**d) for d in docs], "total": total, "page": page, "page_size": page_size}
...
async def _run_upload_and_clean(
    svc: DocumentService, kb_name: str, content_bytes: bytes, filename: str, **kwargs
) -> dict:
    """在线程中执行 upload_and_clean，统一转换服务层异常。"""
    try:
        return await asyncio.to_thread(svc.upload_and_clean, kb_name, content_bytes, filename, **kwargs)
