...
def test_list_documents_success(svc, mock_kb_store, mock_doc_store):
    ...
    mock_doc_store.list_documents.return_value = [{"id": 1, "file_name": "doc.pdf"}]

    result = svc.list_documents("kb1")

    mock_doc_store.list_documents.assert_called_once_with("kb1")
    assert len(result) == 1
...
def test_get_file_path_file_not_found(svc, mock_doc_store):
    ...
    with patch("src.services.document_service._UPLOADS_DIR", fake_dir), pytest.raises(DocumentNotFoundError):
        svc.get_file_path(1, "kb1")
...
def test_get_file_path_success(svc, mock_doc_store):
    ...
    with patch("src.services.document_service._UPLOADS_DIR", fake_dir):
