"""FAQ 业务逻辑：embed/upsert、Excel 导入导出。

从 api/routes/faq.py 提取，路由层仅负责参数解析和响应。
"""
...
def build_faq_workbook(faqs: list[dict] | None = None) -> Workbook:
    ...
    sample_fill = PatternFill(
        start_color="F8F6F2", end_color="F8F6F2", fill_type="solid"
    )
...
def parse_faq_sheet(ws) -> tuple[list[dict], list[dict]]:
    ...
    for row_idx, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
        if all(
            ...
            for v in (row[:4] if len(row) >= 4 else row)
        ):
            continue
...
def batch_embed_and_upsert(
    ...
    for i in range(0, len(rows), batch_size):
        ...
        try:
            ...
            logger.warning(
                "[faq import] batch %d embed/upsert 失败: %s", i // batch_size, e
            )
            for r in batch:
                results[r["faq_id"]] = e
