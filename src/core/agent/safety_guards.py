...
class RuleSafetyGuard(BaseSafetyGuard):
    ...
    def check(self, query: str, generation: str) -> tuple[str, list[str]]:
        ...
        if ("连续三届" in query or "三年" in query) and any(
            term in query for term in ("哪几届", "几届学生", "具体指")
        ):
        ...
        if (
            "专业匹配度说明" in query
            and "符合计算机类专业毕业设计统一基准规范" in query
        ):
        ...
        if "每位指导教师" in query and any(
            term in query for term in ("最多", "多少名", "指导多少")
        ):
        ...
        if (
            ...
            and "正式启动" in query
            and any(
                term in query for term in ("什么时候", "啥时候", "年月日", "12月22")
            )
        ):
        ...
        if any(
            term in query
            for term in ("自己挑题目", "开始选题", "啥时候开始", "什么时候开始")
        ) and any(
            term in query
            for term in ("2026届", "大四上", "发题库", "老师先发题", "选题")
        ):
        ...
        if (
            ...
            and any(
                term in query for term in ("最晚", "什么时候", "时间点", "文件规定")
            )
        ):
        ...
        if "每周不少于1次" in query and any(
            term in query
            for term in ("线上", "腾讯会议", "出差", "有效指导", "过程成绩")
        ):
        ...
        if "任务书作为" in query and any(
            term in query
            for term in ("哪一级组织", "明确界定", "提交主体", "提交时限", "形式要求")
        ):
        ...
        if ("离校前" in query or "离校时间" in query) and (
            "最后一门考试" in query or "1月15" in query
        ):
        ...
        if "任务书" in query and any(
            term in query for term in ("谁来确认", "按时交", "已接收", "超期记录")
        ):
