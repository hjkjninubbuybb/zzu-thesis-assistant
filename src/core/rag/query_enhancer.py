...
class RuleQueryEnhancer(BaseQueryEnhancer):
    ...
    def enhance(self, query: str) -> str:
        ...
        additions: list[str] = []
        stage_related = any(
            term in query for term in ("文献综述", "开题", "中期检查", "初稿", "定稿")
        )
        topic_pool_deadline_related = (
            ...
            and any(
                term in query
                for term in ("最晚", "什么时候", "时间点", "文件规定", "汇总", "公示")
            )
        )
        task_timing_related = (
            any(
                term in query
                for term in ("任务书", "选题", "师生见面", "师生双选", "题库")
            )
            and any(
                term in query
                for term in (
        ...
        if "文献综述" in query:
            additions.append(
                "文献综述 引言 国内外现状 技术分析 问题与挑战 总结 参考文献 中期检查"
            )
        ...
        if reference_policy_file_related:
            additions.append(
                "郑州大学本科毕业论文（设计）工作规定 校教务 2016 10号 文件精神 教务部通知"
            )
        ...
        if "中期检查" in query:
            additions.append(
                "中期检查 指导教师填写评语 检查教师 分组听取学生汇报 第六周"
            )
        if stage_related and any(
            term in query
            for term in ("审核", "权限", "批准", "二级审核", "学院审核", "领导小组")
        ):
            additions.append(
                "指导教师审核 学院管理员 二级审核 学院毕业设计领导小组 监督 检查"
            )
        if any(
            term in query for term in ("格式", "图表", "公式", "摘要", "缩略语", "模板")
        ):
            additions.append("格式规范 图表编号 公式排版 摘要要求 缩略语 首次出现 全称")
        if "初稿" in query:
            additions.append("论文初稿 第十一周 2026.5.11 2026.5.15 上传论文初稿 查重")
        if "开题报告" in query and any(
            term in query for term in ("第几周", "日期范围", "哪几天", "必须", "提交")
        ):
            additions.append("第一周 2026.3.2 2026.3.6 学生开题并提交开题报告")
        if "文献综述" in query and ("中期检查" in query or "中期检查表" in query):
            additions.append(
                "第六周 2026.4.6 2026.4.10 中期检查 学生提交文献综述和中期检查表"
            )
        if "查重" in query or "复制比" in query or "AIGC" in query:
            additions.append(
                "学术不端检测 合格标准 教务部规定 复制比≤30% AIGC生成比例≤40% 进入下一环节"
            )
        if topic_pool_deadline_related:
            additions.append(
                "第七学期期末考试周前 各专业组织教师拟题 专家审核 汇总选题结果报学院教学办"
            )
        if "师生双选" in query and any(
            term in query for term in ("启动", "开始", "立即", "另行通知", "安排依据")
        ):
            additions.append(
                "第十七周–第十九周 2025.12.29–2026.1.16 师生见面 指导教师发布选题 提交任务书"
            )
        if "任务书" in query and any(
            term in query
            for term in ("谁来确认", "按时", "已接收", "超期记录", "指导过程记录表")
        ):
            additions.append(
                "指导教师 检查工作进度 工作质量 指导过程记录表 学生填写指导内容 指导教师审核并签名"
            )
        paper_material_before_proposal = any(
            term in query for term in ("纸质版材料", "纸质材料", "每类材料", "几份")
        ) and any(
            term in query for term in ("师生双选", "开题前", "教学办公室", "指导教师")
        )
        if (
            "任务书" in query
            and any(
            ...
            additions.append(
                "第十七周–第十九周 2025.12.29 2026.1.16 师生见面 指导教师发布选题 第十九周末提交任务书"
            )
        if stage_related and any(
            term in query
            for term in ("第几周", "哪一周", "具体日期", "时间节点", "最晚")
        ):
