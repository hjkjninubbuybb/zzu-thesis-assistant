...
class UserStore:
    ...
    def list_users(
        ...
        with get_conn() as conn:
            with conn.cursor() as cur:
                cur.execute(f"SELECT COUNT(*) AS total FROM users {where}", params)  # noqa: S608
                total = cur.fetchone()["total"]
                cur.execute(
                    f"SELECT * FROM users {where} ORDER BY created_at DESC LIMIT %s OFFSET %s",  # noqa: S608
    ...
    def update_user(self, user_id: int, **kwargs) -> dict | None:
        ...
        with get_conn() as conn:
            with conn.cursor() as cur:
                cur.execute(f"UPDATE users SET {set_clause} WHERE id = %s", values)  # noqa: S608
                conn.commit()
                cur.execute("SELECT * FROM users WHERE id = %s", (user_id,))
                return cur.fetchone()
