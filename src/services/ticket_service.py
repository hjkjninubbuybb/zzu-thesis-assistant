...
class TicketService(BaseService):
    ...
    def list_tickets(self, role: str, user_id: int) -> list[dict]:
        """按角色过滤列出工单。
        ...
        """
        if role == "student":
            return self._ticket_store.list_qa_requests(student_id=user_id)
        if role == "teacher":
            return self._ticket_store.list_qa_requests(mentor_id=user_id)
        # admin 看全部
