...
const h1 = (page: Page, text: string) =>
  page.locator("h1", { hasText: text }).first();
...
test.describe("登录与鉴权", () => {
  ...
  test("TC-LOGIN-02 错误密码被拒", async ({ page }) => {
    ...
    await expect(
      page.locator("text=/Request failed|登录失败|错误/").first(),
...
test.describe("管理端页面冒烟", () => {
  ...
  test("TC-ADM-KB 知识库管理", async ({ page }) => {
    ...
    await expect(
      page.getByRole("button", { name: "新建知识库" }),
    ).toBeVisible();
  });
