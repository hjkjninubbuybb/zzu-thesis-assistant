import { FileText, Loader2, CheckCircle, Clock, AlertCircle, X } from 'lucide-react';
    ...
    <div className="mt-3 space-y-1.5">
      {queue.map((item) => (
        ...
        >
          ...
          {item.status === 'error' && (
            <span className="text-xs text-red-500 shrink-0 max-w-32 truncate" title={item.error}>
              {item.error}
            </span>
          )}
          {item.status === 'pending' && (
