...
export const userService = {
  list: (params?: { role?: string; page?: number; page_size?: number }) => userApi.list(params),
  ...
  resetPassword: (id: number, newPassword: string) => userApi.resetPassword(id, newPassword),
  // student profile
  updateStudentProfile: (id: number, body: Parameters<typeof userApi.updateStudentProfile>[1]) =>
    userApi.updateStudentProfile(id, body),
  // teacher profile
  updateTeacherProfile: (id: number, body: Parameters<typeof userApi.updateTeacherProfile>[1]) =>
  ...
  // mentor relations
  listMentorStudents: (mentorId: number) => userApi.listMentorStudents(mentorId),
