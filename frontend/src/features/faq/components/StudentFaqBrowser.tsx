  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="flex-1 text-sm font-medium text-[#202938]">
          {faq.question}
        </span>
        {faq.category && (
...
export function StudentFaqBrowser() {
  ...
  const { data: allFaqs = [], isLoading } = useQuery({
    queryKey: ["faqs-student", kbName],
    queryFn: () => faqApi.list(kbName),
    enabled: !!kbName,
    select: (data) => data.filter((f) => f.enabled),
  });
  ...
  return (
    <div className="p-7 flex-1 overflow-y-auto glass-card rounded-2xl">
      ...
      <div className="mb-6" style={cardStyle(0)}>
        <h1 className="text-xl font-semibold text-[#202938] tracking-tight">
          常见问题
        </h1>
