...
import { 
  Ticket, Search, MessageSquare, Clock, CheckCircle2, XCircle, 
  ChevronRight, Send, User, Calendar, ExternalLink, Plus, X as XIcon, MessageSquareQuote, Loader2
} from 'lucide-react'
...
function StatusBadge({ status }: { status: QARequestInfo['status'] }) {
  const map = {
    pending: { label: '待处理', icon: Clock, class: 'bg-amber-50 text-amber-600 border-amber-100' },
    replied: { label: '已回复', icon: CheckCircle2, class: 'bg-emerald-50 text-emerald-600 border-emerald-100' },
    closed: { label: '已关闭', icon: XCircle, class: 'bg-slate-50 text-slate-500 border-slate-100' },
  }
  const config = map[status]
  const Icon = config.icon
  return (
    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-medium border ${config.class}`}>
      <Icon size={10} />
      {config.label}
    </span>
  )
...
function TicketDetailModal({ ticket, onClose }: { ticket: QARequestInfo; onClose: () => void }) {
...
function TicketToFaqModal({ 
  ...
  const handleSubmit = async () => {
    if (!kbName) { setError('请选择目标知识库'); return }
    if (!question.trim() || !answer.trim()) { setError('问题和回答不能为空'); return }
      ...
      <div className="glass-card rounded-2xl w-full max-w-md mx-4 overflow-hidden animate-apple-pop shadow-2xl">
        ...
        <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto custom-scrollbar">
          <div className="space-y-1.5">
            <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">目标知识库 <span className="text-red-400">*</span></label>
            <select
              value={kbName}
          ...
          <div className="space-y-1.5">
            <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">分类</label>
            <input
              value={category}
          ...
          <div className="space-y-1.5">
            <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">FAQ 问题 <span className="text-red-400">*</span></label>
            <textarea
              value={question}
          ...
          <div className="space-y-1.5">
            <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">FAQ 回答 <span className="text-red-400">*</span></label>
            <textarea
              value={answer}
...
export default function TicketsPage() {
  ...
  return (
    <div className="px-8 py-8 flex-1 overflow-y-auto glass-card rounded-2xl custom-scrollbar flex flex-col gap-5">
      ...
      <div className="flex items-center justify-between shrink-0" style={settle(0)}>
        ...
        <div className="flex items-center gap-2 p-1.5 bg-[#F2EFE9] rounded-xl border border-[#E8E4DC]">
          {['all', 'pending', 'replied', 'closed'].map(s => (
            ...
            >
              {s === 'all' ? '全部' : s === 'pending' ? '待处理' : s === 'replied' ? '已回复' : '已关闭'}
            </button>
          ))}
        </div>
      </div>
      ...
      {/* 列表 */}
      <div className="flex-1 overflow-y-auto min-h-0 space-y-3 pr-1 custom-scrollbar" style={settle(100)}>
        {isLoading ? (
          <div className="flex items-center justify-center h-32 text-sm text-[#9A9A9A]">加载中...</div>
        ) : filtered.length === 0 ? (
          ...
          filtered.map((t, i) => (
            <div
              ...
              style={{ animation: `appleFadeUp 0.5s cubic-bezier(0.25, 1, 0.5, 1) ${150 + i * 50}ms both` }}
            >
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 transition-transform group-hover:scale-110 ${
                t.status === 'pending' ? 'bg-amber-100 text-amber-600' : 'bg-[#F2EFE9] text-[#8A8A8A]'
              }`}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-1.5">
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-bold font-mono text-slate-400 bg-white/50 px-2 py-0.5 rounded border border-black/5">#{t.id}</span>
                    <StatusBadge status={t.status} />
                  </div>
                  <span className="text-[10px] font-bold text-[#8A8A8A] uppercase tracking-wider">{new Date(t.created_at).toLocaleDateString()}</span>
                </div>
