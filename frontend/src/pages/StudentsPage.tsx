...
function CreateStudentModal({ onClose }: { onClose: () => void }) {
  ...
  const [form, setForm] = useState<
    Omit<UserCreate, "username"> & Partial<StudentProfileCreate>
  >({
    password: "",
    display_name: "",
    role: "student",
      ...
      >
        ...
        {isAdmin && (
          <div className="flex items-center gap-2">
            <button
              onClick={() => userApi.downloadTemplate()}
            ...
            <button
              onClick={() => userApi.exportStudents()}
              className="flex items-center gap-1.5 px-3.5 py-2.5 border border-[#E8E4DC] text-sm text-[#334155] rounded-xl hover:bg-[#F8F6F2] transition-colors"
            >
              <Download size={14} /> 导出
            </button>
                            ...
                            <>
                              ...
                              <button
                                onClick={() => {
                                  if (
                                    confirm(
                                      `确定删除学生 "${u.display_name || u.username}" 吗？`,
                                    )
                                  )
