...
function CreateDialog({
  ...
  const mutation = useMutation({
    mutationFn: () =>
      knowledgeApi.create({ name: name.trim(), description: desc.trim() }),
    onSuccess: () => {
      ...
      <div className="flex-1 min-w-0">
        {kbName ? (
          <>
            ...
            <p
              className={`text-xs mt-0.5 ${isIndigo ? "text-[#4338CA]" : "text-emerald-700"}`}
            >
              {subLabel}
            </p>
          </>
          ...
          {kbs.map((kb, i) => {
            ...
            return (
              <div key={kb.id} style={settle(Math.min(120 + i * 50, 500))}>
                ...
                >
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    {isStudentKb ? (
                      ...
                      >
                        {clearStudentMutation.isPending ? (
                          <Loader2 size={11} className="animate-spin" />
                        ) : (
                          <X size={11} />
                        )}
                ...
                {isExpanded && (
                  <DocumentPanel kbName={kb.name} onToast={showToast} />
                )}
              </div>
            );
          })}
...
// KnowledgeManagementPage 作为 Tab 使用时的命名导出
export { KnowledgeBasePage as DocumentKnowledgeTab };
