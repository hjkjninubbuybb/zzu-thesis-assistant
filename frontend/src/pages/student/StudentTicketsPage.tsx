...
import { 
  Ticket, MessageSquare, Clock, CheckCircle2, XCircle, 
...
function StatusBadge({ status }: { status: QARequestInfo['status'] }) {
  const map = {
    pending: { label: '等待回复', icon: Clock, class: 'bg-amber-50 text-amber-600 border-amber-100' },
    replied: { label: '导师已回复', icon: CheckCircle2, class: 'bg-emerald-50 text-emerald-600 border-emerald-100' },
    closed: { label: '已结束', icon: XCircle, class: 'bg-slate-50 text-slate-500 border-slate-100' },
  }
  const config = map[status]
  const Icon = config.icon
  return (
    <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-bold border uppercase tracking-wider ${config.class}`}>
      <Icon size={10} />
      {config.label}
    </span>
  )
        ...
        <div className="px-8 py-5 border-b border-[#F0EDE8] flex items-center justify-between bg-white/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-[#9A9A9A] font-mono tracking-tighter uppercase">Query Reference: #{ticket.id}</p>
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            {ticket.answer ? (
              ...
              <div className="bg-amber-50/30 rounded-2xl p-6 border border-dashed border-amber-200 text-center">
                ...
                <p className="text-xs text-amber-600 font-medium">导师正在处理中，请耐心等待回复...</p>
              </div>
            )}
            {ticket.replied_at && (
...
export default function StudentTicketsPage() {
  ...
  const { data: tickets, isLoading } = useQuery({
    queryKey: ['my-tickets'],
    queryFn: () => ticketApi.list(), // Student route is already filtered by student_id in backend
  })
  ...
  return (
    <div className="px-8 py-8 flex-1 overflow-y-auto glass-card rounded-2xl custom-scrollbar flex flex-col gap-6">
      ...
      {/* 列表 */}
      <div className="flex-1 overflow-y-auto min-h-0 space-y-4 pr-1 custom-scrollbar" style={settle(100)}>
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-64 text-[#9A9A9A] gap-4 bg-gray-50/50 rounded-3xl border border-dashed border-gray-200">
            ...
            <div className="text-center">
              ...
              <p className="text-[11px] text-gray-400 mt-1">在 AI 对话中点击「求助导师」可发起提问</p>
            </div>
          </div>
        ) : (
          <div className="grid grid-cols-1 gap-3">
            {filtered.map((t, i) => (
              ...
              >
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 transition-transform group-hover:scale-110 shadow-sm border ${
                  t.status === 'pending' 
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-3">
                      <span className="text-[10px] font-bold font-mono text-gray-400 bg-gray-50 px-2 py-0.5 rounded border border-black/5 uppercase">Ref #{t.id}</span>
