  ...
  async (error) => {
    ...
    if (!refreshToken) {
      ...
      window.location.href =
        p === "student" ? "/student/login" : "/admin/login";
      return Promise.reject(error);
    }
...
export const userApi = {
  ...
  updateStudentProfile: (id: number, body: StudentProfileCreate) =>
    client
      .put(`/users/${id}/profile`, { student_profile: body })
      .then((r) => r.data),
  delete: (id: number) =>
  ...
  updateTeacherProfile: (id: number, body: TeacherProfileCreate) =>
    client
      .put(`/users/${id}/profile`, { teacher_profile: body })
      .then((r) => r.data),

  // 师生关系
  listMentorStudents: (mentorId: number) =>
    client
      .get<UserInfo[]>(`/users/mentors/${mentorId}/students`)
      .then((r) => r.data),
  ...
  removeMentorRelation: (mentorId: number, studentId: number) =>
    client
      ...
      .then((r) => r.data),
  getMyMentor: () =>
    client.get<UserInfo>("/users/me/mentor").then((r) => r.data),
...
export const knowledgeApi = {
  ...
  create: (body: KBCreate) =>
    client.post<KBInfo>("/knowledge", body).then((r) => r.data),
  delete: (name: string) =>
    client
      .delete<{ message: string }>(`/knowledge/${name}`)
      .then((r) => r.data),
  // 学生知识库分配
  getActiveKb: () =>
    client.get<ActiveKBInfo | null>("/knowledge/active").then((r) => r.data),
  setActiveKb: (kbName: string) =>
    client
      .put<ActiveKBInfo>("/knowledge/active", { kb_name: kbName })
      .then((r) => r.data),
  clearActiveKb: () =>
  ...
  getAdminKb: () =>
    client
      .get<ActiveKBInfo | null>("/knowledge/admin-active")
      .then((r) => r.data),
  setAdminKb: (kbName: string) =>
    client
      .put<ActiveKBInfo>("/knowledge/admin-active", { kb_name: kbName })
      .then((r) => r.data),
  clearAdminKb: () =>
    client
      .delete<{ message: string }>("/knowledge/admin-active")
      .then((r) => r.data),
};
...
export const documentApi = {
  ...
  update: (kbName: string, docId: number, body: DocUpdate) =>
    client
      .put<DocDetail>(`/document/${kbName}/${docId}`, body)
      .then((r) => r.data),

  reindex: (kbName: string, docId: number) =>
    client
      .post<DocInfo>(`/document/${kbName}/${docId}/reindex`)
      .then((r) => r.data),
  ...
  delete: (kbName: string, docId: number) =>
    client
      .delete<{ message: string }>(`/document/${kbName}/${docId}`)
      .then((r) => r.data),
  ...
  getDownloadToken: (downloadUrl: string) => {
    ...
    return client
      .post<{ token: string; expires_in: number }>(path)
      .then((r) => r.data);
  },
  ...
  confirmClean: (kbName: string, docId: number, content: string) =>
    client.post<ChunkPreviewResult>(
      `/document/${kbName}/${docId}/confirm-clean`,
      { content },
    ),

  confirmIndex: (kbName: string, docId: number) =>
    client.post<ConfirmIndexResult>(
      `/document/${kbName}/${docId}/confirm-index`,
    ),
...
export const configApi = {
  ...
  getApiKey: () =>
    client
      .get<ApiKeyInfo & { api_base_url: string }>("/config/api-key")
      .then((r) => r.data),
...
export const faqApi = {
  ...
  search: (kbName: string, q: string) =>
    client
      .get<FAQSearchResponse>(`/faq/${kbName}/search`, { params: { q } })
      .then((r) => r.data),
  ...
  delete: (kbName: string, id: number) =>
    client
      .delete<{ message: string }>(`/faq/${kbName}/${id}`)
      .then((r) => r.data),
  ...
  summary: () =>
    client.get<AnalyticsSummary>("/analytics/summary").then((r) => {
      ...
      if (
        !d ||
        typeof d !== "object" ||
        !Array.isArray((d as AnalyticsSummary).week_data)
      ) {
...
export const conversationApi = {
  list: (params?: {
    ...
    client
      .get<PaginatedConversations>("/conversation", { params })
      .then((r) => r.data),
  ...
  updateTitle: (convId: number, title: string) =>
    client
      .put<ConversationInfo>(`/conversation/${convId}/title`, { title })
      .then((r) => r.data),
  summarizeTitle: (convId: number) =>
    client
      .post<ConversationInfo>(`/conversation/${convId}/summarize-title`)
      .then((r) => r.data),
  delete: (convId: number) =>
    client
      .delete<{ message: string }>(`/conversation/${convId}`)
      .then((r) => r.data),
  addMessage: (
    convId: number,
    msg: {
    ...
    client
      .post<ConversationMessage>(`/conversation/${convId}/messages`, msg)
      .then((r) => r.data),
...
export const ticketApi = {
  ...
  get: (id: number) =>
    client.get<QARequestInfo>(`/tickets/${id}`).then((r) => r.data),
  create: (body: QARequestCreate) =>
    client.post<QARequestInfo>("/tickets", body).then((r) => r.data),
  reply: (id: number, answer: string) =>
    client
      .post<QARequestInfo>(`/tickets/${id}/reply`, { answer })
      .then((r) => r.data),
  close: (id: number) =>
    client.post<QARequestInfo>(`/tickets/${id}/close`).then((r) => r.data),
};
