...
interface UploadContextType {
  queue: UploadItem[]
  addFiles: (kbName: string, docType: DocType, files: File[], params: UploadParams) => void
...
export function UploadProvider({ children }: { children: React.ReactNode }) {
  ...
  const startLoop = useCallback(async () => {
    ...
    while (true) {
      ...
      updateQueue(prev =>
        prev.map(q => q.id === pending.id ? { ...q, status: 'uploading' as const, progress: 0 } : q)
      )
      ...
      try {
        const doc = await documentApi.upload(
          ...
          pending.file,
          pending.params,
          (pct) => updateQueue(prev =>
            prev.map(q => q.id === pending.id ? { ...q, progress: pct } : q)
          ),
        )
        updateQueue(prev =>
          prev.map(q => q.id === pending.id
            ? { ...q, status: 'done' as const, progress: 100, chunkCount: doc.chunk_count }
            : q
          )
        )
  ...
  const removeItem = useCallback((id: string) => {
    ...
    updateQueue(prev => prev.filter(q => !(q.id === id && q.status !== 'uploading')))
  }, [updateQueue])

  const clearDone = useCallback((kbName: string, docType: DocType) => {
    updateQueue(prev =>
      prev.filter(q => !(q.kbName === kbName && q.docType === docType && (q.status === 'done' || q.status === 'error')))
    )
  }, [updateQueue])
  ...
  return (
    <UploadContext.Provider value={{ queue, addFiles, removeItem, clearDone, isUploading }}>
      {children}
